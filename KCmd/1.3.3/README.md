# [KCmd-1.3.3](https://doc.kaven.xyz/KCmd/1.3.3)
...
| DoubleQuotes    |       | false               |                   |
| SingleQuotes    |       | false               |                   |
| Size            |       | false               | output size first |
| Date            |       | false               | output date first |
...
kcmd list ./ OnlyFiles Size
kcmd list ./ OnlyFiles Date
kcmd list ./ OnlyFiles Size Date
```

### List-Special-Folder

Lists all available special folders on the current system, such as Desktop, Documents, ApplicationData, etc.

#### Syntax(List-Special-Folder)

```sh
kcmd list-special-folder [name:value] ...
```

#### Required Parameters(List-Special-Folder)

none

#### Optional Parameters(List-Special-Folder)

| Name   | Alias | Default | Definition  |
|--------|-------|---------|-------------|
| Folder |       |         | folder Name |

#### Remarks(List-Special-Folder)

none

#### Examples(List-Special-Folder)

```sh
kcmd list-special-folder
kcmd list-special-folder Folder:UserProfile
```
...
- This command is primarily intended for use with [kaven-file-server](https://github.com/kaven-universe/kaven-file-server).  
- If the `Destination` is a directory, ensure that it ends with a trailing slash (`/`).  
- The `Destination` string supports two special placeholders: `{{Version}}` and `{{Now}}`.  
  - **`{{Version}}`**: Inserts the [parsed version](#parse-version) from the current working directory.  
  - **`{{Now}}`**: Inserts the current timestamp. You can specify a custom format, for example: `{{Now:yyyyMMddHHmmssfff}}`.  
...
| Name     | Description                                                |
|----------|------------------------------------------------------------|
| Silent   | Suppresses all output and logs when set to `true`.         |
| Verbose  | Enables detailed logging when set to `true`.               |
| ExitCode | Specifies a 32-bit signed integer [exit code](#exit-code). |
